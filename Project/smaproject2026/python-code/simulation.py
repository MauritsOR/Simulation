    ...
    def setupScenario(self, strategy: int, num_urgent: int, rule: int) -> None:
        ...
        for sess_idx, count in enumerate(slots_per_session):
            ...
            if strategy == 1:
                ...
                # Strategy 3: Urgent slot every 6 elective slots
                current_urgent = 0
                temp_indices = []
                for s in range(start_s, end_s):
                    if (s - start_s - current_urgent) > 0 and (s - start_s - current_urgent) % 6 == 0:
                        if current_urgent < count:
                            temp_indices.append(s)
                            current_urgent += 1
                # If we still need to place slots, put them at the end
